...
def train_and_evaluate(config: ml_collections.ConfigDict):
    ...
    for epoch in range(config.num_epochs):
        ...
        test_batch = (test_ds[64*epoch:64*(epoch+1)]).astype(config.cdtype) / 255.
        ...
        print('eval epoch: {}, loss: {:.4f}, RECON_LOSS: {:.4f}, SFA_LOSS: {:.4f}'.format(epoch + 1, float(metrics['loss']), float(metrics['rec_loss']), float(metrics['sfa_loss'])))
